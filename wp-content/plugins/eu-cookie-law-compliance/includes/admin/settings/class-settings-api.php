...
class TPLIS_CL_Settings_API {
	...
	function callback_head( $args ) {
		...
		echo '<span class="tplis-cl-settings-hr"></span>';
	}
	...
	function callback_text( $args ) {
		...
		$html = sprintf( '<input type="%1$s" class="%2$s-text %3$s-%4$s" id="%3$s[%4$s]" name="%3$s[%4$s]" value="%5$s"/>', $type, $size, $this->name, $args[ 'id' ], $value );
	...
	function callback_checkbox( $args ) {
		...
		$html .= sprintf( '<input type="checkbox" class="checkbox" id="wpuf-%1$s[%2$s]" name="%1$s[%2$s]" value="on" %3$s />', $this->name, $args[ 'id' ], checked( $value, 'on', false ) );
	...
	function callback_radio( $args ) {
		...
		$html = '<fieldset>';
		foreach ( $args[ 'options' ] as $key => $label ) {
			$html .= sprintf( '<label for="%2$s-%3$s-%4$s">', $this->prefix, $this->name, $args[ 'id' ], $key );
			$html .= sprintf( '<input type="radio" id="%2$s-%3$s-%4$s" class="%2$s-%3$s radio" name="%2$s[%3$s]" value="%4$s" %5$s />', $this->prefix, $this->name, $args[ 'id' ], $key, checked( $value, $key, false ) );
			$html .= sprintf( '%1$s</label><br>', $label );
		}
		$html .= $this->get_field_description( $args );
		$html .= '</fieldset>';
    ...
    /**
	 * Displays a basic editor
	 *
	 * @param array   $args settings field args
	 */
	function callback_wysiwyg_basic( $args ) {
	...
	function callback_color( $args ) {
		...
		$html = sprintf( '<input type="text" class="%1$s-text wp-color-picker-field %2$s-%3$s" id="%2$s[%3$s]" name="%2$s[%3$s]" value="%4$s" data-default-color="%5$s" />', $size, $this->name, $args[ 'id' ], $value, $args[ 'std' ] );
		$html .= $this->get_field_description( $args );
